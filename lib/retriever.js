    ...
    if (path.match("zip$")) {
        ...
        stream.on("end", function() {
            ...
            function onEntry(entry) {
                var isSubtitle = (options.supportedExt || ["srt"]).some(function(el) { return entry.getName().match(el + "$") })
                if (foundSubtitle || !isSubtitle) return;
                foundSubtitle = true;
                bufs.push(entry.getData());
            }
            
            if (!foundSubtitle) return callback(new Error("no subtitle file found in zip file "+path));
            onDownloaded();
        });
    } else {
